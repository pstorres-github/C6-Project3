...
const FlightMap = ({displayExtras}) => {
  ...
  function getNewCoordinates() {
    ...
    let longB =
      longA +
      Math.atan2(
        Math.sin(bearing) * Math.sin(distance / radius) * Math.cos(latA),
  ...
  function startRecording() {
    ...
    function doCalculations() {
      ...
      if (currentXSpeed.current === 0 && currentYSpeed.current === 0) return;
      ...
      customUI: ({ onClose }) => {
        return (
          <div className="confirm-modal-container">
            <div className="confirm-modal-header">Confirm Deletion</div>
            ...
            <>
                    ...
                    <Form className="form-initial-location">
                      <div className="form-group">
                        <div className="form-item">
                          ...
                          <div className="form-item inline">
                            <button type="submit" className="map-button inline">
                              Submit
                            </button>
                          </div>
                        </div>
              ...
              <div className="lat-long inline">
                <p className="small-text">
                  Current lat:{" "}
                  <span className="plex">{positionLat.current}&deg;</span>
                  &nbsp; long:{" "}
                  <span className="plex">
            ...
            <div id="mapid">
              ...
              >
                ...
                <Marker key={"start"} position={defaultCenter}>
                  <Tooltip>
                    {" "}
                    Starting Position <br /> Lat: {initialLat.current}, Lng:{" "}
                    {initialLng.current}
                  </Tooltip>
                </Marker>
                ...
                {jobContext.flightPlan &&
                  jobContext.flightPlan.map((markers, index) => (
                    <Marker
                      ...
                      markerIndex={index}
                      key={index}
                      position={[Number(markers.lat), Number(markers.lng)]}
                    >
                      <Tooltip>
                        {" "}
                        Location {index + 1} <br /> Lat: {markers.lat}, Lng:{" "}
                        {markers.lng}
                      </Tooltip>
                    </Marker>
                  ))}
