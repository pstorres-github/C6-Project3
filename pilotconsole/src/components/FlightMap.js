...
const FlightMap = () => {
    ...
    function updateStartCoordinates(lat,lng) {
        ...
        setCoordinates(coordinates => [{lat: Number(lat),lng: Number(lng)}])
    }
    ...
    function calculateDistanceAndBearing(NS_Distance, EW_Distance) {
        ...
        let distance = Math.sqrt(Math.pow(NS_Distance,2)+Math.pow(EW_Distance,2))*recordingInterval/1000
        ...
        let bearing = (NS_Distance >= 0 && EW_Distance < 0) ? (450-angle) : (90-angle)
...

    {
        ...
        let latB = Math.asin(Math.sin(latA)*Math.cos(distance/radius) + Math.cos(latA)*Math.sin(distance/radius)*Math.cos(bearing))
        let longB = longA + Math.atan2(Math.sin(bearing)*Math.sin(distance/radius)*Math.cos(latA),Math.cos(distance/radius)-Math.sin(latA)*Math.sin(latB))
        ...
        // update coordinate array with new coordinates
        setCoordinates(coordinates => [...coordinates, {lat: Number(latBdegrees),lng: Number(longBdegrees)}])
    ...
    function startRecording () 
    {
        ...
        function doCalculations () {
            ...
            calculateDistanceAndBearing(currentXSpeed.current, currentYSpeed.current)
            getNewCoordinates()
    ...
    const handleClearRecording = () =>  updateStartCoordinates(initialLat.current,initialLng.current)
    ...
    return (
        ...
        <LoadScript googleMapsApiKey={GOOGLE_MAP_API_KEY}>
           ...
           <GoogleMap
              ...
              center={{lat:positionLat.current, lng:positionLong.current}}
            >   
